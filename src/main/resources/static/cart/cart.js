...
const allSelectCheckbox = document.querySelector("#allSelectCheckbox"); //전체 선택
const partialDeleteLabel = document.querySelector("#partialDeleteLabel"); //선택 삭제 버튼
...
async function insertProductsfromCart() {
  ...
  for (let i = 0; i < products.length; i++) {
    // 객체 destructuring
    const { _id, title, quantity, imageKey, price } = product[i];
    const imageUrl = await getImageUrl(imageKey);

    const isSelected = selectedIds.includes(_id);

    totalcost[i].textContent = price.toLocaleString() + "원";

    //상품 개수 증감
    plus[i].addEventListener("click", () => {
      let count = parseInt(result[i].textContent);
      count++;
      result[i].textContent = count;
      let totalcostNum = count * price;
